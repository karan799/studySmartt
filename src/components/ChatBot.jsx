    ...
    console.log(message);
      const prompt = prompts[category][selectedValue][level][0];
        ...
        let formattedText = response.text();
         formattedText = formattedText.replace(/(\d+\.\s*[A-Za-z])/g, '<br/>$1');

  // Replace double asterisks with <strong> tags for bold text
  formattedText = formattedText.replace(/\*\*(.*?)\*\*/g, '<br><strong>$1</strong>');

  // Replace "* " with a line break followed by "* " for correct formatting
  formattedText = formattedText.replace(/\* /g, '<br/>* ');

  // Ensure single asterisks are removed (if not intended for line breaks)
      ...
      <section className="white-text center proper-spacing">
        {chats.map((chat, index) => (
          <p key={index} className={chat.role === 'user' ? 'user-msg' : 'ai-msg'}>
            ...
            <ChatResponse content={chat.content} />
